...
The second equivalent MCP query should not resend the full first response, and an agent should not keep searching after enough evidence.

## 给 Agent 的任务

```text
Explain the Model.save persistence spine through save_base and _save_table to the update/insert helpers. Give only the essential chain and files.
```
